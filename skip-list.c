...
#ifdef DEBUG
    printf("\nInserting %d into tree, %d, %d", key, newNode->left->key, newNode->key);
    if (newNode->right)
...
bool tossCoin()
{
    if (_rand_init == 0) { // if thes  seed has not been set
...
NODE* promote(LIST* list, NODE* node)
{
    ...
    if (parentNode == NULL)
        ...
        return insertAfterAbove(parentNode, node, node->key, node->value);
}
